...
# ---------------------------------------------------------------------------
# Public file downloads (generated by the agent)
# ---------------------------------------------------------------------------


@public_router.get("/download/{filename}")
def download_generated_file(filename: str):
    """Serve a previously generated file with a forced download header.

    Content-Disposition is always 'attachment' so the file downloads
    instead of opening in the browser, even for HTML/PDF/CSV.
    """
    ...
    if not full.is_file():
        raise HTTPException(
            ...
            detail="File not found or already expired.",
        )
    return FileResponse(
        full,
        filename=filename,
