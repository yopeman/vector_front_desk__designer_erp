...
export default function ProformaInvoicesPage({ preselectedOrderId }) {
  ...
  const [showTestInvoiceModal, setShowTestInvoiceModal] = useState(false);
  ...
  const handleOpenTestInvoiceModal = () => {
    // Generate TEST-INV-xxxxx format
  ...
  const handleExportTestInvoice = () => {
    try {
      ...
      testInvoiceData.items.forEach(item => {
        ...
        doc.text(String(item.unit_price || 0), 130, y);
  ...
  return (
    <div>
      ...
      {/* Test Invoice Modal */}
      {showTestInvoiceModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl border border-slate-200 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-slate-200 p-6 flex justify-between items-center">
              <h2 className="text-xl font-bold text-slate-800">Test Proforma Invoice</h2>
              <button
                onClick={() => setShowTestInvoiceModal(false)}
                className="text-slate-500 hover:text-slate-700 font-medium flex items-center gap-1 border-none bg-transparent cursor-pointer text-xs"
              >
                <i className="fa-solid fa-xmark"></i>
              </button>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  value={testInvoiceData.client_name}
                  onChange={(e) => setTestInvoiceData({ ...testInvoiceData, client_name: e.target.value })}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 focus:outline-none bg-white"
                >
                  <option value="">Select client</option>
                  {clients.map(client => (
                    <option key={client.id} value={client.name}>{client.name} ({client.client_type})</option>
                  ))}
                </select>
              </div>
