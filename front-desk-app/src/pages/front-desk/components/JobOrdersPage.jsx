...
export default function JobOrdersPage() {
  ...
  const fetchJobOrders = async () => {
    try {
      ...
      let query = supabase
        .from('job_orders')
        .select('*, invoice:invoices(invoice_no, invoice_type, order:orders(*, client:clients(name)))', { count: 'exact' })
        .order('created_at', { ascending: false });

      // Apply search filter
      if (searchQuery) {
        query = query.or(`job_no.ilike.%${searchQuery}%,invoice.invoice_no.ilike.%${searchQuery}%,invoice.order.client.name.ilike.%${searchQuery}%`);
      }
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mt-4">
        <div className="text-sm text-slate-600">
          Showing {(currentPage - 1) * itemsPerPage + 1}-{Math.min(currentPage * itemsPerPage, totalCount)} of {totalCount} job orders
        </div>
