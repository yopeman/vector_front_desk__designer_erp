...
const HireTab = ({ isActive, searchQuery, onDataChange }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      alert('Error adding hire request')
    } else {
  ...
  const handleDelete = async (id) => {
    ...
    if (error) {
      ...
      fetchHires()
      if (onDataChange) onDataChange()
      alert('Hire request deleted successfully!')
    }
  }
  ...
  const handleViewDetails = async (hire) => {
    setSelectedHire(hire)
    setIsDetailModalOpen(true)
  }

  const handleApproveWithDetails = async () => {
    await handleStatusUpdate(selectedHire.id, 'Approved')
    setIsDetailModalOpen(false)
    setSelectedHire(null)
  }

  const handleRejectWithDetails = async () => {
    await handleStatusUpdate(selectedHire.id, 'Rejected')
    setIsDetailModalOpen(false)
    setSelectedHire(null)
  }
  ...
  const handleEdit = (hire) => {
    setFormData({
      request_date: hire.request_date,
      ...
      text_message: hire.text_message || '',
      voice_note: hire.voice_note || '',
      voice_note_url: hire.voice_note_url || '',
      attached_file: hire.attached_file || null,
      attached_file_url: hire.attached_file_url || ''
    })
  ...
  const handleUpdate = async (e) => {
    ...
    if (error) {
      ...
      alert('Error updating hire request')
    } else {
      setIsModalOpen(false)
      setEditingId(null)
      setFormData({
    ...
    <div className="tab-content active space-y-4">
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h3 className="font-bold text-slate-800 text-lg">Letter of Hire</h3>
            <p className="text-xs text-slate-400 mt-0.5">Request hire letters</p>
          </div>
          <button 
            onClick={() => {
              setEditingId(null)
          ...
          <table className="w-full text-left border-collapse dynamic-target-table">
            ...
            <tbody className="divide-y divide-slate-100 text-xs text-slate-600 font-medium">
              {filteredHires.map((hire) => (
                <tr key={hire.id} className="hover:bg-slate-50/60 transition">
                  <td className="p-4 font-semibold">{hire.request_date}</td>
                  <td className="p-4 font-bold text-slate-800">{hire.candidate_name}</td>
                  <td className="p-4">{hire.position}</td>
                  <td className="p-4">{hire.department}</td>
                  <td className="p-4">{hire.start_date}</td>
                  <td className="p-4">
                    <span className={`px-2.5 py-1 rounded-full font-semibold ${
                      hire.status === 'Approved' ? 'bg-primary-50 text-error-600' :
                      hire.status === 'Rejected' ? 'bg-red-50 text-error-600' :
                      hire.status === 'Under Review' ? 'bg-primary-50 text-error-600' :
                    ...
                    <div className="flex gap-2">
                      ...
                      {isCreativeAdmin && (
                        <>
                          ...
                          <button
                            onClick={() => handleStatusUpdate(hire.id, 'Approved')}
                            disabled={hire.status === 'Approved' || hire.status === 'Rejected'}
                            className={`px-2 py-1 rounded transition ${
                              hire.status === 'Approved' || hire.status === 'Rejected'
                          ...
                          <button
                            onClick={() => handleStatusUpdate(hire.id, 'Rejected')}
                            disabled={hire.status === 'Approved' || hire.status === 'Rejected'}
                            className={`px-2 py-1 rounded transition ${
                              hire.status === 'Approved' || hire.status === 'Rejected'
          ...
          <div className="bg-white w-11/12 max-w-lg mx-auto rounded-2xl shadow-2xl z-50 overflow-y-auto max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 text-sm">
                <i className="fa-solid fa-user-plus text-teal-500 mr-2"></i>Letter of Hire
              </h3>
              <button 
                onClick={() => {
            ...
            <form onSubmit={editingId ? handleUpdate : handleSubmit} className="p-5 space-y-4 shadow-inner">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">candidate name</label>
                <input
                  type="text"
                  required
              ...
              <div className="flex justify-end pt-4 border-t border-slate-100">
                <button
                  type="submit"
                  className="px-6 py-2 bg-teal-600 text-white rounded-xl text-xs font-semibold hover:bg-teal-700 transition"
                >
      ...
      {/* Detail Modal */}
      {isDetailModalOpen && selectedHire && (
        <div className="modal opacity-100 pointer-events-auto fixed w-full h-full top-0 left-0 flex items-center justify-center z-50">
          <div 
            className="modal-overlay absolute w-full h-full bg-slate-900/40 backdrop-blur-sm" 
            onClick={() => {
          ...
          <div className="bg-white w-11/12 max-w-2xl mx-auto rounded-2xl shadow-2xl z-50 overflow-y-auto max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 text-sm">
                <i className="fa-solid fa-eye text-error-500 mr-2"></i>Hire Request Details
              </h3>
              <button 
                onClick={() => {
                  setIsDetailModalOpen(false)
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-600">{selectedHire.request_date}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Candidate Name</p>
                  <p className="text-sm font-semibold text-slate-800">{selectedHire.candidate_name}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Position</p>
                  <p className="text-sm text-slate-600">{selectedHire.position}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Department</p>
                  <p className="text-sm text-slate-600">{selectedHire.department}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Start Date</p>
                  <p className="text-sm text-slate-600">{selectedHire.start_date}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Salary</p>
                  <p className="text-sm text-slate-600">{selectedHire.salary}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Employment Type</p>
                  <p className="text-sm text-slate-600">{selectedHire.employment_type}</p>
                </div>
                <div>
                  ...
                  <span className={`px-2.5 py-1 rounded-full font-semibold text-sm ${
                    selectedHire.status === 'Approved' ? 'bg-primary-50 text-error-600' :
                    selectedHire.status === 'Rejected' ? 'bg-red-50 text-error-600' :
                    selectedHire.status === 'Under Review' ? 'bg-primary-50 text-error-600' :
                    'bg-primary-50 text-error-600'
                  }`}>
                    {selectedHire.status}
                  </span>
                ...
                {selectedHire.text_message && (
                  <div className="mb-3 p-3 bg-primary-50 rounded-lg border border-blue-200">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <button 
                        onClick={() => {
                          navigator.clipboard.writeText(selectedHire.text_message)
                    ...
                    <p className="text-sm text-error-600 whitespace-pre-wrap">{selectedHire.text_message}</p>
                  </div>
                )}

                {/* Voice Note */}
                {selectedHire.voice_note && (
                  <div className="mb-3 p-3 bg-purple-50 rounded-lg border border-purple-200">
                    <p className="text-xs font-semibold text-purple-800 mb-2">
                      <i className="fa-solid fa-microphone mr-1"></i>Voice Note
                    </p>
                    <div className="flex items-center gap-3">
                      {selectedHire.voice_note_url ? (
                        <>
                          <audio 
                            ref={(audio) => {
                              if (audio) {
                            ...
                            src={selectedHire.voice_note_url}
                            controls
                      ...
                      <span className="text-xs text-purple-600">{selectedHire.voice_note}</span>
                    </div>
                  </div>
                )}

                {/* Attached File */}
                {selectedHire.attached_file && (
                  <div className="mb-3 p-3 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button 
                        onClick={() => {
                          if (selectedHire.attached_file_url) {
                            const link = document.createElement('a')
                            link.href = selectedHire.attached_file_url
                            link.download = selectedHire.attached_file
                            link.click()
                          } else {
                      ...
                      <span className="text-xs text-green-600">{selectedHire.attached_file}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              {isCreativeAdmin && (
                <div className="border-t border-slate-100 pt-4 flex justify-end gap-3">
                  <button
                    ...
                    disabled={selectedHire.status === 'Approved' || selectedHire.status === 'Rejected'}
                    className={`px-6 py-2 rounded-xl text-xs font-semibold transition ${
                      selectedHire.status === 'Approved' || selectedHire.status === 'Rejected'
      ...
      {/* File Viewer Modal */}
      {isFileViewerOpen && selectedHire?.attached_file && (
        <div className="modal opacity-100 pointer-events-auto fixed w-full h-full top-0 left-0 flex items-center justify-center z-50">
          <div 
            className="modal-overlay absolute w-full h-full bg-slate-900/40 backdrop-blur-sm" 
            onClick={() => {
          ...
          <div className="bg-white w-11/12 max-w-4xl mx-auto rounded-2xl shadow-2xl z-50 overflow-hidden">
            ...
            <div className="p-6">
              {selectedHire.attached_file_url && selectedHire.attached_file_url.startsWith('data:') ? (
                <div className="bg-slate-100 rounded-xl p-8 flex flex-col items-center justify-center min-h-[400px]">
                  {selectedHire.attached_file.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                    <img 
                      src={selectedHire.attached_file_url} 
                      alt={selectedHire.attached_file}
                      className="max-w-full max-h-[500px] object-contain rounded-lg"
                    />
                  ) : (
                    <>
                      <i className="fa-solid fa-file-lines text-6xl text-slate-400 mb-4"></i>
                      <p className="text-sm font-semibold text-slate-700 mb-2">{selectedHire.attached_file}</p>
