...
const TransferTab = ({ isActive, searchQuery, onDataChange }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        requested_department: formData.requested_department,
    ...
    if (error) {
      ...
      alert('Error adding transfer request')
    } else {
  ...
  const handleDelete = async (id) => {
    ...
    if (error) {
      ...
      alert('Error deleting transfer request')
    } else {
      fetchTransfers()
      if (onDataChange) onDataChange()
  ...
  const handleApproveWithDetails = async () => {
    await handleStatusUpdate(selectedTransfer.id, 'Approved')
    setIsDetailModalOpen(false)
    setSelectedTransfer(null)
  }

  const handleRejectWithDetails = async () => {
    await handleStatusUpdate(selectedTransfer.id, 'Rejected')
    setIsDetailModalOpen(false)
    setSelectedTransfer(null)
  }
  ...
  const handleEdit = (transfer) => {
    setFormData({
      ...
      current_position: transfer.current_position,
      requested_department: transfer.requested_department,
      requested_position: transfer.requested_position,
      reason: transfer.reason,
      effective_date: transfer.effective_date,
      text_message: transfer.text_message || '',
      voice_note: transfer.voice_note || '',
      voice_note_url: transfer.voice_note_url || '',
      attached_file: transfer.attached_file || null,
      attached_file_url: transfer.attached_file_url || ''
    })
  ...
  const handleUpdate = async (e) => {
    ...
    if (error) {
      ...
      alert('Error updating transfer request')
    } else {
      setIsModalOpen(false)
      setEditingId(null)
      setFormData({
    ...
    <div className="tab-content active space-y-4">
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h3 className="font-bold text-slate-800 text-lg">Letter of Transfer</h3>
            <p className="text-xs text-slate-400 mt-0.5">Request department transfers</p>
          </div>
          <button 
            onClick={() => {
              setEditingId(null)
            ...
            className="px-4 py-2 bg-orange-600 text-white rounded-xl text-xs font-semibold hover:bg-orange-700 transition"
          >
            ...
            <tbody className="divide-y divide-slate-100 text-xs text-slate-600 font-medium">
              {filteredTransfers.map((transfer) => (
                <tr key={transfer.id} className="hover:bg-slate-50/60 transition">
                  <td className="p-4 font-semibold">{transfer.request_date}</td>
                  <td className="p-4 font-bold text-slate-800">{transfer.employee_name}</td>
                  <td className="p-4">{transfer.current_department}</td>
                  <td className="p-4">{transfer.requested_department}</td>
                  <td className="p-4">{transfer.effective_date}</td>
                  <td className="p-4">
                    <span className={`px-2.5 py-1 rounded-full font-semibold ${
                      transfer.status === 'Approved' ? 'bg-primary-50 text-error-600' :
                      transfer.status === 'Rejected' ? 'bg-red-50 text-error-600' :
                      transfer.status === 'Under Review' ? 'bg-primary-50 text-error-600' :
                      ...
                      {isCreativeAdmin && (
                        <>
                          ...
                          <button
                            ...
                            disabled={transfer.status === 'Approved' || transfer.status === 'Rejected'}
          ...
          <div className="bg-white w-11/12 max-w-lg mx-auto rounded-2xl shadow-2xl z-50 overflow-y-auto max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 text-sm">
                <i className="fa-solid fa-right-left text-orange-500 mr-2"></i>Letter of Transfer
              </h3>
              <button 
                onClick={() => {
            ...
            <form onSubmit={editingId ? handleUpdate : handleSubmit} className="p-5 space-y-4 shadow-inner">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">current department</label>
                  <input
                    type="text"
                    required
                    value={formData.current_department}
                    onChange={(e) => setFormData({...formData, current_department: e.target.value})}
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-primary-500 focus:outline-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">requested department</label>
                  <input
                    type="text"
                    required
              ...
              <div className="flex justify-end pt-4 border-t border-slate-100">
                <button
                  type="submit"
                  className="px-6 py-2 bg-orange-600 text-white rounded-xl text-xs font-semibold hover:bg-orange-700 transition"
                >
      ...
      {isDetailModalOpen && selectedTransfer && (
        <div className="modal opacity-100 pointer-events-auto fixed w-full h-full top-0 left-0 flex items-center justify-center z-50">
          ...
          <div className="bg-white w-11/12 max-w-2xl mx-auto rounded-2xl shadow-2xl z-50 overflow-y-auto max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 text-sm">
                <i className="fa-solid fa-eye text-error-500 mr-2"></i>Transfer Request Details
              </h3>
              <button 
                onClick={() => {
                  setIsDetailModalOpen(false)
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-600">{selectedTransfer.request_date}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Employee Name</p>
                  <p className="text-sm font-semibold text-slate-800">{selectedTransfer.employee_name}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Current Department</p>
                  <p className="text-sm text-slate-600">{selectedTransfer.current_department}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Current Position</p>
                  <p className="text-sm text-slate-600">{selectedTransfer.current_position}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Requested Department</p>
                  <p className="text-sm text-slate-600">{selectedTransfer.requested_department}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Requested Position</p>
                  <p className="text-sm text-slate-600">{selectedTransfer.requested_position}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Effective Date</p>
                  <p className="text-sm text-slate-600">{selectedTransfer.effective_date}</p>
                </div>
                <div>
                  ...
                  <span className={`px-2.5 py-1 rounded-full font-semibold text-sm ${
                    selectedTransfer.status === 'Approved' ? 'bg-primary-50 text-error-600' :
                    selectedTransfer.status === 'Rejected' ? 'bg-red-50 text-error-600' :
                    selectedTransfer.status === 'Under Review' ? 'bg-primary-50 text-error-600' :
                    'bg-primary-50 text-error-600'
                  }`}>
              ...
              {/* Reason */}
              <div>
                <p className="text-xs font-bold text-slate-500 uppercase mb-1">Reason for Transfer</p>
                <p className="text-sm text-slate-600 bg-slate-50 p-3 rounded-lg">{selectedTransfer.reason}</p>
              </div>
                ...
                {selectedTransfer.text_message && (
                  <div className="mb-3 p-3 bg-primary-50 rounded-lg border border-blue-200">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <button 
                        onClick={() => {
                          navigator.clipboard.writeText(selectedTransfer.text_message)
                    ...
                    <p className="text-sm text-error-600 whitespace-pre-wrap">{selectedTransfer.text_message}</p>
                  </div>
                )}

                {/* Voice Note */}
                {selectedTransfer.voice_note && (
                  <div className="mb-3 p-3 bg-purple-50 rounded-lg border border-purple-200">
                    <p className="text-xs font-semibold text-purple-800 mb-2">
                      <i className="fa-solid fa-microphone mr-1"></i>Voice Note
                    </p>
                    <div className="flex items-center gap-3">
                      {selectedTransfer.voice_note_url ? (
                        <>
                          <audio 
                            ref={(audio) => {
                              if (audio) {
                            ...
                            src={selectedTransfer.voice_note_url}
                            controls
                      ...
                      <span className="text-xs text-purple-600">{selectedTransfer.voice_note}</span>
                    </div>
                  </div>
                )}

                {/* Attached File */}
                {selectedTransfer.attached_file && (
                  <div className="mb-3 p-3 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button 
                        onClick={() => {
                          if (selectedTransfer.attached_file_url) {
                            const link = document.createElement('a')
                            link.href = selectedTransfer.attached_file_url
                            link.download = selectedTransfer.attached_file
                            link.click()
                          } else {
                      ...
                      <span className="text-xs text-green-600">{selectedTransfer.attached_file}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              {isCreativeAdmin && (
                <div className="border-t border-slate-100 pt-4 flex justify-end gap-3">
                  <button
                    ...
                    disabled={selectedTransfer.status === 'Approved' || selectedTransfer.status === 'Rejected'}
      ...
      {/* File Viewer Modal */}
      {isFileViewerOpen && selectedTransfer?.attached_file && (
        <div className="modal opacity-100 pointer-events-auto fixed w-full h-full top-0 left-0 flex items-center justify-center z-50">
          <div 
            className="modal-overlay absolute w-full h-full bg-slate-900/40 backdrop-blur-sm" 
            onClick={() => {
          ...
          <div className="bg-white w-11/12 max-w-4xl mx-auto rounded-2xl shadow-2xl z-50 overflow-hidden">
            ...
            <div className="p-6">
              {selectedTransfer.attached_file_url && selectedTransfer.attached_file_url.startsWith('data:') ? (
                <div className="bg-slate-100 rounded-xl p-8 flex flex-col items-center justify-center min-h-[400px]">
                  {selectedTransfer.attached_file.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                    <img 
                      src={selectedTransfer.attached_file_url} 
                      alt={selectedTransfer.attached_file}
                      className="max-w-full max-h-[500px] object-contain rounded-lg"
                    />
                  ) : (
                    <>
                      <i className="fa-solid fa-file-lines text-6xl text-slate-400 mb-4"></i>
                      <p className="text-sm font-semibold text-slate-700 mb-2">{selectedTransfer.attached_file}</p>
