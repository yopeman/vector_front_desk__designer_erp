...
const ExperienceTab = ({ isActive, searchQuery, onDataChange }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      alert('Error adding experience request')
    } else {
      ...
      alert('Experience request submitted successfully!')
    }
  }

  const handleDelete = async (id) => {
    if (!confirm('Are you sure you want to delete this experience request?')) return
    ...
    if (error) {
      ...
      alert('Error deleting experience request')
    } else {
      fetchExperiences()
      if (onDataChange) onDataChange()
      alert('Experience request deleted successfully!')
    }
  }
  ...
  const handleApproveWithDetails = async () => {
    await handleStatusUpdate(selectedExperience.id, 'Approved')
    setIsDetailModalOpen(false)
    setSelectedExperience(null)
  }

  const handleRejectWithDetails = async () => {
    await handleStatusUpdate(selectedExperience.id, 'Rejected')
    setIsDetailModalOpen(false)
    setSelectedExperience(null)
  }
  ...
  const handleEdit = (experience) => {
    setFormData({
      request_date: experience.request_date,
      employee_name: experience.employee_name,
      ...
      voice_note: experience.voice_note || '',
      voice_note_url: experience.voice_note_url || '',
      attached_file: experience.attached_file || null,
      attached_file_url: experience.attached_file_url || ''
    })
  ...
  const handleUpdate = async (e) => {
    ...
    if (error) {
      ...
      alert('Error updating experience request')
    } else {
      setIsModalOpen(false)
      setEditingId(null)
      setFormData({
      ...
      fetchExperiences()
      if (onDataChange) onDataChange()
      alert('Experience request updated successfully!')
    }
  }
    ...
    <div className="tab-content active space-y-4">
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h3 className="font-bold text-slate-800 text-lg">Letter of Experience</h3>
            <p className="text-xs text-slate-400 mt-0.5">Request experience letters</p>
          </div>
          <button 
            onClick={() => {
              setEditingId(null)
          ...
          <table className="w-full text-left border-collapse dynamic-target-table">
            ...
            <tbody className="divide-y divide-slate-100 text-xs text-slate-600 font-medium">
              {filteredExperiences.map((experience) => (
                <tr key={experience.id} className="hover:bg-slate-50/60 transition">
                  <td className="p-4 font-semibold">{experience.request_date}</td>
                  <td className="p-4 font-bold text-slate-800">{experience.employee_name}</td>
                  <td className="p-4">{experience.position}</td>
                  <td className="p-4">{experience.department}</td>
                  <td className="p-4">{experience.purpose}</td>
                  <td className="p-4">
                    <span className={`px-2.5 py-1 rounded-full font-semibold ${
                      experience.status === 'Approved' ? 'bg-primary-50 text-error-600' :
                      experience.status === 'Rejected' ? 'bg-red-50 text-error-600' :
                      experience.status === 'Under Review' ? 'bg-primary-50 text-error-600' :
                    ...
                    <div className="flex gap-2">
                      ...
                      {isCreativeAdmin && (
                        <>
                          ...
                          <button
                            onClick={() => handleStatusUpdate(experience.id, 'Approved')}
                            disabled={experience.status === 'Approved' || experience.status === 'Rejected'}
                          ...
                          <button
                            onClick={() => handleStatusUpdate(experience.id, 'Rejected')}
                            disabled={experience.status === 'Approved' || experience.status === 'Rejected'}
                            className={`px-2 py-1 rounded transition ${
                              experience.status === 'Approved' || experience.status === 'Rejected'
          ...
          <div className="bg-white w-11/12 max-w-lg mx-auto rounded-2xl shadow-2xl z-50 overflow-y-auto max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 text-sm">
                <i className="fa-solid fa-file-lines text-green-500 mr-2"></i>Letter of Experience
              </h3>
              <button 
                onClick={() => {
            ...
            <form onSubmit={editingId ? handleUpdate : handleSubmit} className="p-5 space-y-4 shadow-inner">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">employment start date</label>
                  <input
                    type="date"
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">employment end date</label>
                  <input
                    type="date"
                    required
              ...
              <div className="flex justify-end pt-4 border-t border-slate-100">
                <button
                  type="submit"
                  className="px-6 py-2 bg-green-600 text-white rounded-xl text-xs font-semibold hover:bg-green-700 transition"
                >
      ...
      {isDetailModalOpen && selectedExperience && (
        <div className="modal opacity-100 pointer-events-auto fixed w-full h-full top-0 left-0 flex items-center justify-center z-50">
          ...
          <div className="bg-white w-11/12 max-w-2xl mx-auto rounded-2xl shadow-2xl z-50 overflow-y-auto max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 text-sm">
                <i className="fa-solid fa-eye text-error-500 mr-2"></i>Experience Request Details
              </h3>
              <button 
                onClick={() => {
                  setIsDetailModalOpen(false)
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-600">{selectedExperience.request_date}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Employee Name</p>
                  <p className="text-sm font-semibold text-slate-800">{selectedExperience.employee_name}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Position</p>
                  <p className="text-sm text-slate-600">{selectedExperience.position}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Department</p>
                  <p className="text-sm text-slate-600">{selectedExperience.department}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Employment Start Date</p>
                  <p className="text-sm text-slate-600">{selectedExperience.employment_start_date}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Employment End Date</p>
                  <p className="text-sm text-slate-600">{selectedExperience.employment_end_date}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Purpose</p>
                  <p className="text-sm text-slate-600">{selectedExperience.purpose}</p>
                </div>
                <div>
                  ...
                  <span className={`px-2.5 py-1 rounded-full font-semibold text-sm ${
                    selectedExperience.status === 'Approved' ? 'bg-primary-50 text-error-600' :
                    selectedExperience.status === 'Rejected' ? 'bg-red-50 text-error-600' :
                    selectedExperience.status === 'Under Review' ? 'bg-primary-50 text-error-600' :
                    'bg-primary-50 text-error-600'
                  }`}>
              ...
              <div className="border-t border-slate-100 pt-4">
                ...
                {/* Text Message */}
                {selectedExperience.text_message && (
                  <div className="mb-3 p-3 bg-primary-50 rounded-lg border border-blue-200">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <button 
                        onClick={() => {
                          navigator.clipboard.writeText(selectedExperience.text_message)
                    ...
                    <p className="text-sm text-error-600 whitespace-pre-wrap">{selectedExperience.text_message}</p>
                  </div>
                )}

                {/* Voice Note */}
                {selectedExperience.voice_note && (
                  <div className="mb-3 p-3 bg-purple-50 rounded-lg border border-purple-200">
                    <p className="text-xs font-semibold text-purple-800 mb-2">
                      <i className="fa-solid fa-microphone mr-1"></i>Voice Note
                    </p>
                    <div className="flex items-center gap-3">
                      {selectedExperience.voice_note_url ? (
                        <>
                          <audio 
                            ref={(audio) => {
                              if (audio) {
                            ...
                            src={selectedExperience.voice_note_url}
                            controls
                      ...
                      <span className="text-xs text-purple-600">{selectedExperience.voice_note}</span>
                    </div>
                  </div>
                )}

                {/* Attached File */}
                {selectedExperience.attached_file && (
                  <div className="mb-3 p-3 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button 
                        onClick={() => {
                          if (selectedExperience.attached_file_url) {
                            const link = document.createElement('a')
                            link.href = selectedExperience.attached_file_url
                            link.download = selectedExperience.attached_file
                            link.click()
                          } else {
                      ...
                      <span className="text-xs text-green-600">{selectedExperience.attached_file}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              {isCreativeAdmin && (
                <div className="border-t border-slate-100 pt-4 flex justify-end gap-3">
                  <button
                    ...
                    disabled={selectedExperience.status === 'Approved' || selectedExperience.status === 'Rejected'}
                    className={`px-6 py-2 rounded-xl text-xs font-semibold transition ${
                      selectedExperience.status === 'Approved' || selectedExperience.status === 'Rejected'
      ...
      {/* File Viewer Modal */}
      {isFileViewerOpen && selectedExperience?.attached_file && (
        <div className="modal opacity-100 pointer-events-auto fixed w-full h-full top-0 left-0 flex items-center justify-center z-50">
          <div 
            className="modal-overlay absolute w-full h-full bg-slate-900/40 backdrop-blur-sm" 
            onClick={() => {
          ...
          <div className="bg-white w-11/12 max-w-4xl mx-auto rounded-2xl shadow-2xl z-50 overflow-hidden">
            ...
            <div className="p-6">
              {selectedExperience.attached_file_url && selectedExperience.attached_file_url.startsWith('data:') ? (
                <div className="bg-slate-100 rounded-xl p-8 flex flex-col items-center justify-center min-h-[400px]">
                  {selectedExperience.attached_file.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                    <img 
                      src={selectedExperience.attached_file_url} 
                      alt={selectedExperience.attached_file}
                      className="max-w-full max-h-[500px] object-contain rounded-lg"
                    />
                  ) : (
                    <>
                      <i className="fa-solid fa-file-lines text-6xl text-slate-400 mb-4"></i>
                      <p className="text-sm font-semibold text-slate-700 mb-2">{selectedExperience.attached_file}</p>
