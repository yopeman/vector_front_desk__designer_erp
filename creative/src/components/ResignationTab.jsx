...
const ResignationTab = ({ isActive, searchQuery, onDataChange }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      alert(`Error adding resignation request: ${error.message}`)
    } else {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Are you sure you want to delete this resignation request?')) return
    ...
    if (error) {
      ...
      alert('Error deleting resignation request')
    } else {
      fetchResignations()
      if (onDataChange) onDataChange()
  ...
  const handleViewDetails = async (resignation) => {
    setSelectedResignation(resignation)
    setIsDetailModalOpen(true)
  }

  const handleApproveWithDetails = async () => {
    await handleStatusUpdate(selectedResignation.id, 'Approved')
    setIsDetailModalOpen(false)
    setSelectedResignation(null)
  }

  const handleRejectWithDetails = async () => {
    await handleStatusUpdate(selectedResignation.id, 'Rejected')
    setIsDetailModalOpen(false)
    setSelectedResignation(null)
  }
  ...
  const handleEdit = (resignation) => {
    setFormData({
      application_date: resignation.application_date,
      ...
      text_message: resignation.text_message || '',
      voice_note: resignation.voice_note || '',
      voice_note_url: resignation.voice_note_url || '',
      attached_file: resignation.attached_file || null,
      attached_file_url: resignation.attached_file_url || ''
    })
  ...
  const handleUpdate = async (e) => {
    ...
    if (error) {
      ...
      alert('Error updating resignation request')
    } else {
      setIsModalOpen(false)
      setEditingId(null)
      setFormData({
      ...
      fetchResignations()
      if (onDataChange) onDataChange()
      alert('Resignation request updated successfully!')
    }
  }
    ...
    <div className="tab-content active space-y-4">
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h3 className="font-bold text-slate-800 text-lg">Letter of Resignation</h3>
            <p className="text-xs text-slate-400 mt-0.5">Submit resignation requests</p>
          </div>
          <button 
            onClick={() => {
              setEditingId(null)
          ...
          <table className="w-full text-left border-collapse dynamic-target-table">
            ...
            <tbody className="divide-y divide-slate-100 text-xs text-slate-600 font-medium">
              {filteredResignations.map((resignation) => (
                <tr key={resignation.id} className="hover:bg-slate-50/60 transition">
                  <td className="p-4 font-semibold">{resignation.application_date}</td>
                  <td className="p-4 font-bold text-slate-800">{resignation.employee_name}</td>
                  <td className="p-4">{resignation.position}</td>
                  <td className="p-4">{resignation.department}</td>
                  <td className="p-4">{resignation.last_working_day}</td>
                  <td className="p-4">
                    <span className={`px-2.5 py-1 rounded-full font-semibold ${
                      resignation.status === 'Approved' ? 'bg-primary-50 text-error-600' :
                      resignation.status === 'Rejected' ? 'bg-red-50 text-error-600' :
                      resignation.status === 'Under Review' ? 'bg-primary-50 text-error-600' :
                      ...
                      {isCreativeAdmin && (
                        <>
                          ...
                          <button
                            ...
                            disabled={resignation.status === 'Approved' || resignation.status === 'Rejected'}
                          ...
                          <button
                            onClick={() => handleStatusUpdate(resignation.id, 'Rejected')}
                            disabled={resignation.status === 'Approved' || resignation.status === 'Rejected'}
                            className={`px-2 py-1 rounded transition ${
                              resignation.status === 'Approved' || resignation.status === 'Rejected'
          ...
          <div className="bg-white w-11/12 max-w-lg mx-auto rounded-2xl shadow-2xl z-50 overflow-y-auto max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 text-sm">
                <i className="fa-solid fa-file-signature text-error-500 mr-2"></i>Letter of Resignation
              </h3>
              <button 
                onClick={() => {
                  setEditingId(null)
            ...
            <form onSubmit={editingId ? handleUpdate : handleSubmit} className="p-5 space-y-4 shadow-inner">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">application date</label>
                <input
                  type="date"
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">last working day</label>
                  <input
                    type="date"
                    required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">reason for resignation</label>
                <textarea
                  required
                  rows="3"
      ...
      {isDetailModalOpen && selectedResignation && (
        <div className="modal opacity-100 pointer-events-auto fixed w-full h-full top-0 left-0 flex items-center justify-center z-50">
          <div 
            className="modal-overlay absolute w-full h-full bg-slate-900/40 backdrop-blur-sm" 
            onClick={() => {
          ...
          <div className="bg-white w-11/12 max-w-2xl mx-auto rounded-2xl shadow-2xl z-50 overflow-y-auto max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 text-sm">
                <i className="fa-solid fa-eye text-error-500 mr-2"></i>Resignation Request Details
              </h3>
              <button 
                onClick={() => {
                  setIsDetailModalOpen(false)
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-600">{selectedResignation.application_date}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Employee Name</p>
                  <p className="text-sm font-semibold text-slate-800">{selectedResignation.employee_name}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Position</p>
                  <p className="text-sm text-slate-600">{selectedResignation.position}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Department</p>
                  <p className="text-sm text-slate-600">{selectedResignation.department}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Last Working Day</p>
                  <p className="text-sm text-slate-600">{selectedResignation.last_working_day}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase mb-1">Notice Period</p>
                  <p className="text-sm text-slate-600">{selectedResignation.notice_period}</p>
                </div>
              </div>

              {/* Reason */}
              <div>
                <p className="text-xs font-bold text-slate-500 uppercase mb-1">Reason for Resignation</p>
                <p className="text-sm text-slate-600 bg-slate-50 p-3 rounded-lg">{selectedResignation.reason}</p>
              </div>
              ...
              <div className="border-t border-slate-100 pt-4">
                ...
                {/* Text Message */}
                {selectedResignation.text_message && (
                  <div className="mb-3 p-3 bg-primary-50 rounded-lg border border-blue-200">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <button 
                        onClick={() => {
                          navigator.clipboard.writeText(selectedResignation.text_message)
                    ...
                    <p className="text-sm text-error-600 whitespace-pre-wrap">{selectedResignation.text_message}</p>
                  </div>
                )}

                {/* Voice Note */}
                {selectedResignation.voice_note && (
                  <div className="mb-3 p-3 bg-purple-50 rounded-lg border border-purple-200">
                    <p className="text-xs font-semibold text-purple-800 mb-2">
                      <i className="fa-solid fa-microphone mr-1"></i>Voice Note
                    </p>
                    <div className="flex items-center gap-3">
                      {selectedResignation.voice_note_url ? (
                        <>
                          <audio 
                            ref={(audio) => {
                              if (audio) {
                            ...
                            src={selectedResignation.voice_note_url}
                            controls
                      ...
                      <span className="text-xs text-purple-600">{selectedResignation.voice_note}</span>
                    </div>
                  </div>
                )}

                {/* Attached File */}
                {selectedResignation.attached_file && (
                  <div className="mb-3 p-3 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button 
                        onClick={() => {
                          if (selectedResignation.attached_file_url) {
                            const link = document.createElement('a')
                            link.href = selectedResignation.attached_file_url
                            link.download = selectedResignation.attached_file
                            link.click()
                          } else {
                      ...
                      <span className="text-xs text-green-600">{selectedResignation.attached_file}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              {isCreativeAdmin && (
                <div className="border-t border-slate-100 pt-4 flex justify-end gap-3">
                  <button
                    ...
                    disabled={selectedResignation.status === 'Approved' || selectedResignation.status === 'Rejected'}
                    className={`px-6 py-2 rounded-xl text-xs font-semibold transition ${
                      selectedResignation.status === 'Approved' || selectedResignation.status === 'Rejected'
      ...
      {/* File Viewer Modal */}
      {isFileViewerOpen && selectedResignation?.attached_file && (
        <div className="modal opacity-100 pointer-events-auto fixed w-full h-full top-0 left-0 flex items-center justify-center z-50">
          <div 
            className="modal-overlay absolute w-full h-full bg-slate-900/40 backdrop-blur-sm" 
            onClick={() => {
          ...
          <div className="bg-white w-11/12 max-w-4xl mx-auto rounded-2xl shadow-2xl z-50 overflow-hidden">
            ...
            <div className="p-6">
              {selectedResignation.attached_file_url && selectedResignation.attached_file_url.startsWith('data:') ? (
                <div className="bg-slate-100 rounded-xl p-8 flex flex-col items-center justify-center min-h-[400px]">
                  {selectedResignation.attached_file.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                    <img 
                      src={selectedResignation.attached_file_url} 
                      alt={selectedResignation.attached_file}
                      className="max-w-full max-h-[500px] object-contain rounded-lg"
                    />
                  ) : (
                    <>
                      <i className="fa-solid fa-file-lines text-6xl text-slate-400 mb-4"></i>
                      <p className="text-sm font-semibold text-slate-700 mb-2">{selectedResignation.attached_file}</p>
