...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      if (channel) {
        financeClient.removeChannel(channel);
      }
    };
  }, [selectedUser]);
  ...
  const fetchMessages = async (otherUserId: string) => {
    try {
      ...
      messagesWithFiles.forEach((msg: any) => {
        (msg.attached_file_ids || []).forEach((id: string) => fileIds.add(id));
      });

      let filesMap: Record<string, any> = {};
      if (fileIds.size > 0) {
        const { data: filesData } = await financeClient
          .from('files')
          .select('id, name, path, mime_type, file_size')
          .in('id', Array.from(fileIds));

        if (filesData) {
          filesData.forEach((f: any) => { filesMap[f.id] = f; });
        }
      }

      const enrichedMessages = messagesWithFiles.map((msg: any) => ({
        ...msg,
        attached_files: (msg.attached_file_ids || []).map((id: string) => filesMap[id]).filter(Boolean)
      }));
    ...
    try {
      ...
      if (attachments.length > 0) {
        for (const file of attachments) {
          ...
          const { data: uploadData, error: upErr } = await financeClient.storage
            .from('documents')
          ...
          const { data: fileData, error: fileErr } = await financeClient
            .from('files')
            .insert({
              name: file.name,
              path: storedPath,
                        ...
                        <div style={{
                          ...
                          {msg.attached_file_ids && msg.attached_file_ids.length > 0 && (
                            <div style={{ marginTop: msg.text ? '8px' : 0 }}>
                              {(msg.attached_files || []).map((file: any) => (
                                <div
                                  key={file.id}
